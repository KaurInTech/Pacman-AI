...
def foodHeuristic(state, problem):
    ...
    def union(parent, rank, start, end):
        xroot = findParent(parent, start) 
        yroot = findParent(parent, end) 
    ...
    distance_graph = []
    for i in range(len(unvisited_foods)-1):
        for j in range(i+1, len(unvisited_foods)):
            distance_graph.append([unvisited_foods[i], unvisited_foods[j], util.manhattanDistance(unvisited_foods[i], unvisited_foods[j])])

    heuristic_cost = 0
    i = 0
    e = 0

    distance_graph = sorted(distance_graph, key=lambda x:x[2])

    parentOfNode = {}
    rank = {}

    for food in unvisited_foods:
        parentOfNode[food] = None
        rank[food] = 0

    while e < len(unvisited_foods) - 1:
        start, end, cost = distance_graph[i]
        i += 1
        parentOfStart = findParent(parentOfNode, start)
        parentOfEnd = findParent(parentOfNode, end)

        if parentOfStart != parentOfEnd:
            e += 1
            heuristic_cost += cost
            union(parentOfNode, rank, start, end)

    closest_food = min([util.manhattanDistance(position, food) for food in unvisited_foods])

    return closest_food + heuristic_cost
